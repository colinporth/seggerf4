...
//}}}
//{{{
void sdRamTest1 (int iterations, uint16_t* addr, uint32_t len) {
  ...
  while (iterations--) {
    ...
    auto readAddress = addr;
    for (uint32_t i = 0; i < len/2; i++) {
      *writeAddress++ = i & 0xFFFF;
      uint16_t read = *readAddress++;

      if (read == (i & 0xFFFF)) {
        printf ("ok %p read:%x == %x\n", readAddress, read, i & 0xFFFF);
        readOk++;
        }
      else {
        printf ("error %p read:%x != %x\n", readAddress, read, i & 0xFFFF);
...
//}}}
//{{{
void sdRamTest2 (int iterations, uint16_t* addr, uint32_t len) {
